...
const TimelineTab = ({ onCategorySelect, setSelectedSubCatId }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch the first API
        const response1 = await fetch("https://irdback.vercel.app/categories", {
          cache: "force-cache",
        });
        if (!response1.ok) {
          throw new Error(
        ...
        setFilteredCategories(categori); // Initially, show all categories
  ...
  useEffect(() => {
    ...
    if (searchTerm === "") {
      setFilteredCategories(categories); // If no search term, show all categories
    } else {
  ...
  return (
    <div className="bg-white w-[300px] rounded-lg space-y-3 h-[600px] overflow-x-auto ">
      ...
      <div className="px-2">
        ...
        <div className="relative">
          <input
            ...
            onChange={handleCategorySearch} // Update search term on change
